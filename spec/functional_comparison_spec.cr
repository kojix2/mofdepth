...
def run_tanudepth(args : Array(String), prefix : String, temp_dir : String, test_bam : String) : Process::Status
...
describe "Functional comparison with mosdepth" do
  ...
  describe "basic coverage calculation" do
    it "produces identical per-base output" do
      ...
      tanudepth_status = run_tanudepth([] of String, "#{temp_dir}/tanudepth_test", temp_dir, test_bam)

      mosdepth_status.success?.should be_true
      tanudepth_status.success?.should be_true

      # Compare per-base output
      mosdepth_file = "#{temp_dir}/mosdepth_test.per-base.bed.gz"
      tanudepth_file = "#{temp_dir}/tanudepth_test.per-base.bed.gz"

      File.exists?(mosdepth_file).should be_true
      File.exists?(tanudepth_file).should be_true

      # For now, just check that both files exist and are non-empty
      File.size(mosdepth_file).should be > 0
      File.size(tanudepth_file).should be > 0
    end
    ...
    it "produces identical summary output" do
      ...
      tanudepth_status = run_tanudepth([] of String, "#{temp_dir}/tanudepth_summary", temp_dir, test_bam)

      mosdepth_status.success?.should be_true
      tanudepth_status.success?.should be_true

      mosdepth_summary = "#{temp_dir}/mosdepth_summary.mosdepth.summary.txt"
      tanudepth_summary = "#{temp_dir}/tanudepth_summary.mosdepth.summary.txt"

      File.exists?(mosdepth_summary).should be_true
      File.exists?(tanudepth_summary).should be_true
    end
  end
  ...
  describe "flag filtering" do
    it "handles exclude flag filtering" do
      ...
      tanudepth_status = run_tanudepth(["-F", "4"], "#{temp_dir}/tanudepth_flag", temp_dir, test_bam)
      ...
      File.exists?("#{temp_dir}/tanudepth_flag.per-base.bed.gz").should be_true
    end
    ...
    it "handles include flag filtering" do
      ...
      tanudepth_status = run_tanudepth(["-i", "2"], "#{temp_dir}/tanudepth_include", temp_dir, test_bam)

      mosdepth_status.success?.should be_true
      tanudepth_status.success?.should be_true

      File.exists?("#{temp_dir}/mosdepth_include.per-base.bed.gz").should be_true
      File.exists?("#{temp_dir}/tanudepth_include.per-base.bed.gz").should be_true
    end
  end
  ...
  describe "quantization" do
    it "handles quantization identically" do
      ...
      tanudepth_status = run_tanudepth(["-q", "0:1:1000"], "#{temp_dir}/tanudepth_quant", temp_dir, test_bam)

      mosdepth_status.success?.should be_true
      tanudepth_status.success?.should be_true

      File.exists?("#{temp_dir}/mosdepth_quant.quantized.bed.gz").should be_true
      File.exists?("#{temp_dir}/tanudepth_quant.quantized.bed.gz").should be_true
    end
  end
  ...
  describe "region processing" do
    it "handles window-based regions" do
      ...
      tanudepth_status = run_tanudepth(["-b", "100"], "#{temp_dir}/tanudepth_window", temp_dir, test_bam)

      mosdepth_status.success?.should be_true
      tanudepth_status.success?.should be_true

      File.exists?("#{temp_dir}/mosdepth_window.regions.bed.gz").should be_true
      File.exists?("#{temp_dir}/tanudepth_window.regions.bed.gz").should be_true
    end
    ...
    it "handles BED file regions" do
      ...
      tanudepth_status = run_tanudepth(["-b", bed_file], "#{temp_dir}/tanudepth_bed", temp_dir, test_bam)

      mosdepth_status.success?.should be_true
      tanudepth_status.success?.should be_true

      File.exists?("#{temp_dir}/mosdepth_bed.regions.bed.gz").should be_true
      File.exists?("#{temp_dir}/tanudepth_bed.regions.bed.gz").should be_true
    end
  end
  ...
  describe "threshold processing" do
    it "handles threshold calculations" do
      ...
      tanudepth_status = run_tanudepth(["-T", "0,1,2", "-b", "100"], "#{temp_dir}/tanudepth_thresh", temp_dir, test_bam)

      mosdepth_status.success?.should be_true
      tanudepth_status.success?.should be_true

      File.exists?("#{temp_dir}/mosdepth_thresh.thresholds.bed.gz").should be_true
      File.exists?("#{temp_dir}/tanudepth_thresh.thresholds.bed.gz").should be_true
    end
  end
  ...
  describe "fast mode" do
    it "handles fast mode processing" do
      ...
      tanudepth_status = run_tanudepth(["-x"], "#{temp_dir}/tanudepth_fast", temp_dir, test_bam)

      mosdepth_status.success?.should be_true
      tanudepth_status.success?.should be_true

      File.exists?("#{temp_dir}/mosdepth_fast.per-base.bed.gz").should be_true
      File.exists?("#{temp_dir}/tanudepth_fast.per-base.bed.gz").should be_true
    end
  end
  ...
  describe "fragment mode" do
    it "handles fragment mode processing" do
      ...
      tanudepth_cmd = Process.run("crystal",
        ["run", "src/depth.cr", "--", "-a", "#{temp_dir}/tanudepth_frag", fragment_bam],
        chdir: Dir.current,
        env: {"PWD" => temp_dir},
        output: Process::Redirect::Close,
        error: Process::Redirect::Close)
      ...
      tanudepth_cmd.success?.should be_true

      File.exists?("#{temp_dir}/mosdepth_frag.per-base.bed.gz").should be_true
      File.exists?("#{temp_dir}/tanudepth_frag.per-base.bed.gz").should be_true
    end
  end
  ...
  describe "error handling" do
    it "handles missing chromosome gracefully" do
      ...
      tanudepth_status = run_tanudepth(["-c", "nonexistent"], "#{temp_dir}/tanudepth_missing", temp_dir, test_bam)

      # Both should fail with non-zero exit code
      mosdepth_status.success?.should be_false
      tanudepth_status.success?.should be_false
    end
    ...
    it "handles invalid arguments" do
      ...
      tanudepth_status = run_tanudepth(["--invalid-option"], "#{temp_dir}/tanudepth_invalid", temp_dir, test_bam)
